...
tab0, tab1, tab2, tab3, tab4 = st.tabs(["Geral", "Etnia", "Gêneros", "Desempenho de notas", "Desempenho em matérias"])
st.set_option('deprecation.showPyplotGlobalUse', False) # para evitar um warning 
...
with tab0:
    ...
    plt.grid()
    st.pyplot() #se tivesse figura tinha que colocar dentro do parêntenses para aparecer no streamlit
...
with  tab1:
    ...
    #pode mostrar uma tabela se quiser
...
with tab3:
    ...
    st.write("Estatísticas básicas do grupo de estudos feminino:")
    df_female.describe().T

    st.write("Estatísticas básicas do grupo de estudos masculino:")
    df_male.describe().T

    media = round(df_female['avg score'].mean())
    st.write(f" Analisando as estatísticas, o grupo feminino se desempenhou melhor que o masculino com uma média de score de {media}%.")

with tab4:

    st.write("### Análise de desempenho de notas por disciplina")
    
    #mostra o código formatado
    cod_python = """
    ...
    plt.show()
    """
    st.code(cod_python, language='python')
